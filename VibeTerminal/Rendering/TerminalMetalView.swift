...
struct TerminalMetalView: NSViewRepresentable {
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        func draw(in view: MTKView) {
            ...
            if needsRender || viewModel.hasPendingUpdates {
...
class TerminalViewModel: ObservableObject {
    ...
    private(set) var hasPendingUpdates: Bool = false
...
class GlyphCache {
    ...
    // 纹理配置 - 扩大到 4096，支持更多字符
    private let atlasSize: Int = 4096
    private let glyphSize: CGFloat = 48  // 减小字形尺寸以容纳更多字符
    private var glyphsPerRow: Int {
        Int(atlasSize / Int(glyphSize))
    }
    private let maxGlyphs: Int  // 最大字符数
    ...
    init?(device: MTLDevice) {
        ...
        self.maxGlyphs = (4096 / 48) * (4096 / 48)  // 约 7100 个字符
    ...
    private func uploadGlyphPixels(_ pixels: [UInt8], to blitEncoder: MTLBlitCommandEncoder, at x: Int, y: Int) {
        let bytesPerRow = Int(glyphSize) * 4
        let bytesPerImage = Int(glyphSize) * 4  // 2D 纹理
        let size = MTLSize(width: Int(glyphSize), height: Int(glyphSize), depth: 1)
    ...
    private func cacheCharacterSync(_ char: Character) {
        ...
        if let pixels = renderGlyphToPixels(char) {
            // 直接上传到纹理（初始化阶段可以同步）
            let bytesPerRow = Int(glyphSize) * 4
            let bytesPerImage = Int(glyphSize) * 4
            let size = MTLSize(width: Int(glyphSize), height: Int(glyphSize), depth: 1)
    ...
    private func cacheCharacterAsync(_ char: Character) {
        ...
        let hasSpace = (nextY + Int(glyphSize)) < atlasSize
    ...
    private func renderGlyphToPixels(_ char: Character) -> [UInt8]? {
        ...
        let bytesPerRow = Int(glyphSize) * 4
        let pixelDataSize = Int(glyphSize) * Int(glyphSize) * 4
