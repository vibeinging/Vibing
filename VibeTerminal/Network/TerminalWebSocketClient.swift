...
final class TerminalWebSocketClient: NSObject {
    ...
    func createSession(request: CreateSessionRequest) async throws -> String {
        return try await withCheckedThrowingContinuation { continuation in
            stateQueue.async { [weak self] in
                ...
                Task {
                    do {
                        try await self.sendFrame(.createSession(request))

                        // 设置超时
                        Task {
                            try? await Task.sleep(nanoseconds: UInt64(self.config.connectionTimeout * 1_000_000_000))
                            if self.sessionCreationContinuations.removeValue(forKey: requestId) != nil {
                                continuation.resume(throwing: TerminalClientError.connectionTimeout)
                            }
                        }
                    } catch {
    ...
    /// 创建 Shell 会话
    func createShellSession() async throws -> String {
        try await createSession(request: .shell())
    }
