    ...
    func render(in view: MTKView) {
        ...
        if let cursorPipeline = cursorPipelineState {
            ...
            if cursorInfo.visible {
                ...
                let cursorInstance = buildCursorInstance(from: state)
...
class GlyphCache {
    ...
    init?(device: MTLDevice) {
        ...
        guard let menloFont = NSFont(name: "Menlo", size: fontSize) ??
                              NSFont(name: "Courier", size: fontSize) ??
                              NSFont.monospacedSystemFont(ofSize: fontSize, weight: .regular) else {
            return nil
        }
