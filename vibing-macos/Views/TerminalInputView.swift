...
class TerminalInputNSView: NSView {
    ...
    override func mouseDown(with event: NSEvent) {
        NSLog("[Vibing-Input] mouseDown - requesting first responder")
        window?.makeFirstResponder(self)
    }

    override func viewDidMoveToWindow() {
        super.viewDidMoveToWindow()
        NSLog("[Vibing-Input] viewDidMoveToWindow, window=\(window != nil)")
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            guard let self = self else { return }
            let result = self.window?.makeFirstResponder(self)
            NSLog("[Vibing-Input] Auto first responder result: \(result ?? false), isFirstResponder=\(self.window?.firstResponder === self)")
        }
    }

    override func keyDown(with event: NSEvent) {
        NSLog("[Vibing-Input] keyDown: keyCode=\(event.keyCode), chars=\(event.characters ?? "nil")")

        guard let handler = onKeyPress else {
            NSLog("[Vibing-Input] No handler!")
        ...
        NSLog("[Vibing-Input] handled=\(handled)")
