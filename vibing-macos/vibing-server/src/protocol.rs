...
pub enum Frame {
    ...
    /// 原始 PTY 输出字节流（SwiftTerm 客户端使用）
    RawOutput(SessionId, Vec<u8>),
...
impl fmt::Display for Frame {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Frame::RawOutput(sid, data) => write!(f, "RawOutput({}, {} bytes)", sid, data.len()),
            Frame::Negotiate(req) => write!(f, "Negotiate(protocol={}, v={})", req.protocol, req.version),
...
/// - [1字节] 帧类型
/// - [4字节] 数据长度 (小端序)
/// - [N字节] 数据内容 (JSON 序列化，与 Swift 客户端兼容)
///
/// RawOutput 使用自定义二进制编码（避免 JSON base64 开销）：
/// - [0x16][4B total_len][2B session_id_len][session_id bytes][raw PTY bytes]
pub fn encode_frame(frame: &Frame) -> Vec<u8> {
    // RawOutput 使用自定义二进制编码
    if let Frame::RawOutput(session_id, raw_data) = frame {
        let sid_bytes = session_id.as_bytes();
        let sid_len = sid_bytes.len() as u16;
        let total_len = (2 + sid_bytes.len() + raw_data.len()) as u32;

        let mut result = Vec::with_capacity(5 + total_len as usize);
        result.push(0x16); // RawOutput type marker
        result.extend_from_slice(&total_len.to_le_bytes());
        result.extend_from_slice(&sid_len.to_le_bytes());
        result.extend_from_slice(sid_bytes);
        result.extend_from_slice(raw_data);
        return result;
    }

    // 其他帧使用 serde_json 序列化（与 Swift 客户端兼容）
    let frame_data = match serde_json::to_vec(frame) {
        Ok(data) => data,
        Err(_) => return vec![0xFF, 0x00, 0x00, 0x00, 0x00],
    };
    ...
    let type_marker = match frame {
        ...
        Frame::Error(_) => 0x15,
        Frame::RawOutput(_, _) => unreachable!(), // handled above
        Frame::Negotiate(_) => 0x17,
        Frame::Ping => 0x20,
        Frame::Pong => 0x21,
    };
...
pub fn decode_frame(data: &[u8]) -> Result<Frame, ProtocolError> {
    ...
    // RawOutput 使用自定义二进制解码
    if frame_type == 0x16 {
        if frame_data.len() < 2 {
            return Err(ProtocolError::IncompleteData);
        }
        let sid_len = u16::from_le_bytes([frame_data[0], frame_data[1]]) as usize;
        if frame_data.len() < 2 + sid_len {
            return Err(ProtocolError::IncompleteData);
        }
        let session_id = String::from_utf8_lossy(&frame_data[2..2 + sid_len]).to_string();
        let raw_data = frame_data[2 + sid_len..].to_vec();
        return Ok(Frame::RawOutput(session_id, raw_data));
    }

    // 验证类型标记
    match frame_type {
        0x01..=0x05 | 0x10..=0x17 | 0x20..=0x21 => {}
        _ => return Err(ProtocolError::UnknownFrameType(frame_type)),
    }

    // 先尝试 JSON 反序列化（与 Swift 客户端兼容），再回退到 bincode
    serde_json::from_slice(frame_data)
        .map_err(|e| ProtocolError::SerializationError(e.to_string()))
        .or_else(|_| bincode::deserialize(frame_data).map_err(ProtocolError::from))
}
...
mod tests {
    ...
    fn test_color_palette() {
        ...
        let color = Color::from_8bit(52); // index 52 = 16 + 36 = (1, 0, 0) in cube
    ...
    #[test]
    fn test_encode_decode_raw_output() {
        let raw_data = vec![0x1b, 0x5b, 0x31, 0x6d, 0x48, 0x65, 0x6c, 0x6c, 0x6f]; // ESC[1mHello
        let frame = Frame::RawOutput("session-789".to_string(), raw_data.clone());
        let encoded = encode_frame(&frame);
        ...
        if let Frame::RawOutput(sid, data) = decoded {
            assert_eq!(sid, "session-789");
            assert_eq!(data, raw_data);
        } else {
            panic!("Expected RawOutput frame");
        }
    }

    #[test]
    fn test_encode_decode_negotiate() {
        let frame = Frame::Negotiate(NegotiateRequest {
            protocol: "raw_stream".to_string(),
            version: 2,
        });
        let encoded = encode_frame(&frame);
