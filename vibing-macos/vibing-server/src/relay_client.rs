...
pub struct RelayClient {
    ...
    // 事件通道（relay 状态变化通知）
    event_tx: Arc<Mutex<Option<mpsc::Sender<RelayEvent>>>>,
}
...
impl RelayClient {
    ...
    /// 与 iOS RelayCrypto.swift 保持一致：SHA256(session_id)
                    ...
                    Ok(Message::Close(_)) => {
                        ...
                        *peer_connected.lock().await = false;
                        if let Some(tx) = event_tx.lock().await.as_ref() {
                            let _ = tx.send(RelayEvent::PeerDisconnected).await;
                        }
                        break;
                    }
                    Err(e) => {
                        ...
                        *peer_connected.lock().await = false;
                        if let Some(tx) = event_tx.lock().await.as_ref() {
                            let _ = tx.send(RelayEvent::Error(e.to_string())).await;
                        }
                        break;
                    }
                    _ => {}
            ...
            match msg_type {
                ...
                "peer_connected" => {
                    ...
                    *peer_connected.lock().await = true;
                    if let Some(tx) = event_tx.lock().await.as_ref() {
                        let _ = tx.send(RelayEvent::PeerConnected).await;
                    }
                }
                "error" => {
                    ...
                    if let Some(tx) = event_tx.lock().await.as_ref() {
                        let _ = tx.send(RelayEvent::Error(error_msg.to_string())).await;
                    }
                }
                "pong" => {
