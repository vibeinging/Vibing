...
enum SettingSection: Int, CaseIterable {
    ...
    var title: String {
        ...
        case .terminal: return "终端设置"
        case .cursor: return "光标设置"
...
class SettingsViewController: UIViewController {
    ...
    // Cursor Settings
    private var cursorStyle: CursorStyle {
        get {
            let raw = UserDefaults.standard.string(forKey: "cursorStyle") ?? CursorStyle.block.rawValue
            return CursorStyle.allCases.first { $0.rawValue == raw } ?? .block
        }
        set {
...
extension SettingsViewController: UITableViewDataSource {
    ...
    private func cellForCursorRow(row: CursorRow, tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        ...
        case .style:
            let cell = tableView.dequeueReusableCell(withIdentifier: OptionCell.identifier, for: indexPath) as! OptionCell
            let options = CursorStyle.allCases.map { $0.rawValue }
            cell.configure(title: row.title, options: options, selectedIndex: CursorStyle.allCases.firstIndex(of: cursorStyle) ?? 0)
            cell.onValueChange = { [weak self] index in
                self?.cursorStyle = CursorStyle.allCases[index]
            }
            return cell
        }
    }
