...
func encodeFrame(_ frame: Frame) -> Data {
    ...
    do {
        ...
        var result = Data()
        result.append(0xFF)
        result.append(UInt32(0).littleEndian)
        return result
    }
    ...
    result.append(len.littleEndian)
...
/// 编码帧数据（JSON 格式，与服务器 bincode 兼容的结构）
private func encodeFrameData(_ frame: Frame) throws -> Data {
    // 将帧转换为可编码的字典
    let encodable: Any
    switch frame {
    case .input(let sessionId, let data):
        encodable = ["Input": [sessionId, data.base64EncodedString()]]

    case .subscribe(let sessionIds):
        encodable = ["Subscribe": sessionIds]

    case .createSession(let req):
        encodable = ["CreateSession": req]

    case .resizeSession(let sessionId, let cols, let rows):
        encodable = ["ResizeSession": [sessionId, cols, rows]]

    case .closeSession(let sessionId):
        encodable = ["CloseSession": sessionId]

    case .sessionOutput(let sessionId, let frame):
        encodable = ["SessionOutput": [sessionId, frame]]

    case .cursorUpdate(let cursor):
        encodable = ["CursorUpdate": cursor]

    case .modeUpdate(let mode):
        encodable = ["ModeUpdate": mode]

    case .sessionCreated(let sessionId):
        encodable = ["SessionCreated": sessionId]

    case .sessionClosed(let sessionId):
        encodable = ["SessionClosed": sessionId]
...
func decodeFrame(_ data: Data) throws -> Frame {
    ...
    let len = UInt32(littleEndian: data[1..<5].withUnsafeBytes { $0.load(as: UInt32.self) }) as Int
...
private func decodeFrameData(_ frameType: UInt8, data: Data) throws -> Frame {
    ...
    case FrameTypeMarker.createSession.rawValue:
        ...
        let req = CreateSessionRequest(command: command, args: args, cwd: cwd, env: envTuples)
