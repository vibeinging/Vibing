...
// MARK: - WebSocket Client Property
extension ConnectionViewController {
    private static var webSocketClientKey: UInt8 = 0
    private static var sessionIdKey: UInt8 = 0

    var webSocketClient: WebSocketClient? {
        get { objc_getAssociatedObject(self, &Self.webSocketClientKey) as? WebSocketClient }
        set { objc_setAssociatedObject(self, &Self.webSocketClientKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }

    var currentSessionId: String? {
        get { objc_getAssociatedObject(self, &Self.sessionIdKey) as? String }
        set { objc_setAssociatedObject(self, &Self.sessionIdKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
}
...
extension ConnectionViewController {
    ...
    func connect() {
        ...
        guard let url = URL(string: serverAddress) else {
            print("Invalid server URL: \(serverAddress)")
            connectionState = .disconnected
            return
        }

        var config = WebSocketClientConfig.default
...
extension ConnectionViewController: WebSocketClientDelegate {

    func webSocketDidConnect(_ client: WebSocketClient) {
        print("WebSocket connected, creating shell session...")
        connectionState = .connected

        // Send create_session as JSON text message
    ...
    func webSocketDidDisconnect(_ client: WebSocketClient, error: Error?) {
        ...
        if connectionState != .disconnected {
            connectionState = autoReconnect ? .reconnecting : .disconnected
        }
    }
    ...
    func webSocket(_ client: WebSocketClient, didReceiveFrame frame: Frame) {
        ...
        case .sessionCreated(let sessionId):
            ...
            client.handleSessionCreated(sessionId)
        ...
        case .sessionOutput(let sessionId, let screenFrame):
            print("Session output for \(sessionId): seq=\(screenFrame.seq), \(screenFrame.dirtyRegions.count) dirty regions")
            // Terminal rendering would be handled here by a TerminalViewController

        case .cursorUpdate(let cursor):
            print("Cursor update: (\(cursor.x), \(cursor.y)) visible=\(cursor.visible)")

        case .modeUpdate(let mode):
            print("Mode update: \(mode.mode) active=\(mode.active)")
