...
class WebSocketClient: NSObject {
    ...
    /// 纯文本消息回调（JSON 控制消息如 session_list）
    var onTextMessage: ((String) -> Void)?

    /// Session 输出回调
    var onSessionOutput: ((String, ScreenFrame) -> Void)?

    /// Session Created Observer (used by async connect helper)
    private var sessionCreatedObserver: SessionCreatedObserver?

    // MARK: - 初始化

    convenience init(serverURL: String, config: WebSocketClientConfig = .default) {
    ...
    init(url: URL, config: WebSocketClientConfig = .default) {
        ...
        opQueue.name = "com.vibing.websocket.session"
        opQueue.underlyingQueue = DispatchQueue(label: "com.vibing.websocket.session.queue")
    ...
    // MARK: - JSON Text Message (for server text protocol)
    ...
    /// The server expects plain JSON text for create_session, input, and subscribe commands.
    func sendTextMessage(_ json: [String: Any]) {
        guard isConnected else {
            print("Cannot send text message: not connected")
            return
        }

        guard let data = try? JSONSerialization.data(withJSONObject: json),
              let text = String(data: data, encoding: .utf8) else {
            print("Failed to serialize JSON text message")
            return
        }
    ...
    /// Send input text to a session via JSON text message.
    func sendTextInput(sessionId: String, text: String) {
        sendTextMessage([
            "type": "input",
    ...
    private func handleTextMessage(_ text: String) {
        // 检查是否是 JSON 控制消息（session_list, shell_info 等）
    ...
    private func handleReconnectSuccess() {
        ...
        // 重新订阅会话 via JSON text protocol
        let sessionIds = Array(subscribedSessions)
        if !sessionIds.isEmpty {
            subscribeToSessions(sessionIds)
        }
    ...
    private func flushPendingInputs() {
        Task { [weak self] in
            ...
            for sessionIds in subscribes {
                self.subscribeToSessions(sessionIds)
            }
        }
    }
}
...
extension WebSocketClient {
    ...
    /// 创建默认 Shell 会话via JSON text protocol
    func createShellSession(cols: UInt16 = 80, rows: UInt16 = 24) {
        let request = CreateSessionRequest.shell(cols: cols, rows: rows)
        sendTextMessage(request.toJSON())
    }

    /// 创建 Shell 会话并返回 sessionId
    /// 注意：这需要等待 sessionCreated 帧返回，实际实现中通过委托回调
    func createShellSessionWithId(cols: UInt16 = 80, rows: UInt16 = 24) async throws -> String {
        return try await withCheckedThrowingContinuation { continuation in
            ...
            Task {
                ...
                if self.sessionCreatedObserver != nil {
                    self.sessionCreatedObserver = nil
    ...
    /// 调整会话终端大小 (uses binary frame protocol)
    ...
    /// 创建命令会话 via JSON text protocol
    func createCommandSession(_ command: String, args: [String] = [], cols: UInt16 = 80, rows: UInt16 = 24) {
        let request = CreateSessionRequest.command(command, args: args, cols: cols, rows: rows)
        sendTextMessage(request.toJSON())
    }

    /// Subscribe to session outputs via JSON text protocol
    func subscribeToSessions(_ sessionIds: [String]) {
        sendTextMessage([
            "type": "subscribe",
